...
Version history:
   ...
   1.2.1
      date : Mar 3, 2020
      changes: 
         * error handling option with try/catch statements added to help in identifying problematic 
           time steps when files are read
...
try:
    ...
    if pFuncName != '***':
        ...
        for i in range(0,n):
            try:
                ...
                errTxt = errTxt+"Error! Unable to read surface pressure file at t = "+'{:g}'.format(tSurf[i])+"\n"
        print("\r{0}%".format(int(float(i)/n*100)))
        ...
        if (pttFuncName != "***"):  # read P0 from monitored data
            ...
            for i in range(0,len(tSurf)):
                ...
                #    print("idx = "+str(idx))
                    ...
                    errTxt = errTxt+"Error! Unable to read p0 pressure file at t = "+'{:g}'.format(tSurf[i])+"\n"
        ...
        for tProb in UrefTimes:
            try:
                ...
                errTxt = errTxt+"Error! Unable to read reference velocity file at t = "+tProb+"\n"
        ...
        for uSectName in uSectNames:
            ...
            for i in range(0,n):
                try:
                    ...
                    errTxt = errTxt+"Error! Unable to velocity file of "+uSectName+" at t = "+'{:g}'.format(tSurf[i])+"\n"
            print("\r{0} %".format(int(float(i)/n*100)))
            print(" Done! Reading data from file. \n\n")
    ...
    errTxt = errTxt+"Error! Some unknown error occured"
finally:
    print(errTxt)
    	
    print("\nDone!! \n***check for error in the lines before this)\n")
    
    eTime = time.time()
    print("Elapsed time = {:g} secs\n\n\n".format(eTime-sTime))
