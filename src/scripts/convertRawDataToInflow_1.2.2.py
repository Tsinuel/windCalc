...
if not os.path.exists(outDir+'/inlet'):
    os.makedirs(outDir+'/inlet')
tMin = 6.0
tMax = 100
...
del rawData[0]
del rawData[0]
pointsFile = open(outDir+'/inlet/points', 'w')
pointsFile.write("\n"+str(len(rawData))+"\n(")
for r in rawData:
...
existingTimes = os.listdir(outDir+'/inlet/')
existingTimes.remove('points')
...
for t in timeNames:
    if ((float(t) >= tMin) & (float(t) <= tMax)) :
        ...
        if float(tNew) < 0.0:
            ...
            continue
        if not tNew in existingTimes: #os.path.exists(outDir+'/inlet/'+tNew):
            os.makedirs(outDir+'/inlet/'+tNew)
        else:
        ...
        del rawData[0]
        del rawData[0]
        uFile = open(outDir+'/inlet/'+tNew+"/U", 'w')
        uFile.write("\n"+str(len(rawData))+"\n(")
        for r in rawData:
...
print("Writing inflowDict file to "+ outDir +". \n")

inflowDictFile = open(outDir+'/inflowDict', 'w')
...
print("Done! Writing inflowDict file to "+ outDir +". \n")
