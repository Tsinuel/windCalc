...
# ptt_P0_ID = 53              # pitot point ID among the profile points
...
# timeRange = [27, 100]   # range of time to consider
...
# if (c%(i+1)):
    print("\r{0}%".format(round(float(i/n),4)*100))
print("\r{0}%".format(round(float(i)/n,2)*100))
...
cPfile = open(caseDir + '/postProcessing/Cp_' + pFuncName + '_' + patchName + '_' + '{:g}'.format(tSurf[0]) +'_' + '{:g}'.format(tSurf[-1]) + '.vtk','w')
